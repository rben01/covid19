...
This section contains an interactive plot showing the number of new cases and deaths, total and per capita, in the 48 continental U.S. states and Washington, D.C.
States that are light gray on a given day had no new cases/deaths that day.
The colorbars are logarithmically scaled; each pair of major ticks has eight minor ticks between them (ten including the major ticks), which means the minor ticks between a×b^n^ and a×b^n+1^ are located at a×b^n^(bk-k+9)/9 for k=1, 2, ..., 7, 8. +
The date slider can be dragged to view a specific date.
...
****
There is a known issue where if you leave your mouse cursor stationary over a state while the animation plays, the hover tip's values will not update. To work around this, you can jiggle your mouse while the animation plays.
****

[NOTE]
--
The interactive timeline takes quite a while to load.
If your device or browser isn't well suited for interactivity, a static video showing the timeline is available link:{timeline-video-dir}/dod_diffs.mp4[here]. +
