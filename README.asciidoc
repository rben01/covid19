...
:icon-interactive-cummulative: ↗️
...
Finally, the per-capita numbers assume a constant population within a region.
This assumption is problematic; for instance, early on, many New Yorkers left the state for elsewhere in the U.S.
If a state's true population decreases significantly, then its per-capita numbers will be artificually deflated.
Correspondingly, if a state's true population increases, then its per-capita numbers will be artificually inflated.
...
[[graphs-timeline-interactive]]
=== {icon-interactive-timeline} Interactive Timeline of New Cases Per Day

This section contains two interactive 2x2 grid-plots. Both grid-plots have four quadrants showing how the number of cases and deaths, total and per capita, has progressed in the 48 continental U.S. states and Washington, D.C.
The first grid-plot shows these numbers cummulatively since coronavirus was first confirmed in the U.S.
The second one shows how these numbers increased each day.
For instance, New York had 6 cases on March 4 and 22 on March 5; the top-left quadrant of the first grid-plot would show 22 for New York on March 5, while that of the second grid-plot would show 16 (22 minus 6). +
States that are light gray on a given day had no cases/deaths that day.
...
Also, note that because the per-capita numbers in the two grid-plots have such different magnitudes, they have different denominators (10,000 vs 100,000). +
...
The "playback speed" buttons are self explanatory.

****
There is a known issue where if you leave your mouse cursor stationary over a state while the animation plays, the hover tooltip's values will not update. To work around this, you can jiggle your mouse while the animation plays. I am working on fixing this issue.
****

[NOTE]
--
The interactive timeline takes quite a while to load.
If your device or browser isn't well suited for interactivity, a static video showing the progression day-by-day differences is available link:{timeline-video-dir}/dod_diffs.mp4[here]. +
If your device is small you may not see anything below; try rotating your device or visiting this site on a larger device.
...
==== {icon-interactive-cummulative} Cummulative
[pass]
++++
...
| {bullet-start-date}
| The dates on the graphs' x-axes represent data collected in the time span from the most recent occurrence midnight to the given date.
...
By labeling data with the right endpoint of the data collection period, data can be represented faithfully regardless of whether or not it represents a full 24 hour data collection period (i.e., whether it is from today or from earlier). +
...
| {bullet-gridlines}
| [[pow-10-ticks]]The y-axis of the graphs is log scaled.
