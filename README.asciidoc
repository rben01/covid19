...
The colorbars are logarithmically scaled; each pair of major ticks has eight minor ticks between them (ten including the major ticks), which means the minor ticks between a×b^n^ and a×b^n+1^ are located at a×b^n^(bk-k+9)/9 for k=1, 2, ..., 7, 8. (Substitute a=1, b=10 and you'll arrive at <<pow-10-ticks,the same minor ticks as above>>.) +
The date slider can be dragged to pick a specific date.
