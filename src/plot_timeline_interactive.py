...
def make_daybyday_interactive_timeline(
    ...
    *,
    ...
    value_col: str,
    transform_df_func: Callable[[pd.DataFrame], pd.DataFrame],
    ...
    selected_data_df: pd.DataFrame = geo_df.merge(
        df, how="inner", on=Columns.TWO_LETTER_STATE_CODE,
    )[
        [
            Columns.TWO_LETTER_STATE_CODE,
            Columns.DATE,
    ...
    values_mins_maxs = (
        selected_data_df[selected_data_df[value_col] > 0]
    ...
    selected_data_df.loc[_per_cap_rows, value_col] *= percap_pow10s.loc[_per_cap_rows]
    ...
    selected_data_df[value_col] = selected_data_df[max_date_str]
    selected_data_df[FAKE_DATE_COL] = max_date_str

    selected_data_df[COLOR_COL] = np.where(
        selected_data_df[value_col] > 0, selected_data_df[value_col], "NaN"
    )
...
def make_usa_daybyday_interactive_timeline(
    ...
    *,
    ...
    selected_data_df: pd.DataFrame = usa_states_geo_df.merge(
    ...
    selected_data_df = selected_data_df.merge(
        usa_states_geo_df[[Columns.TWO_LETTER_STATE_CODE, LONG_COL, LAT_COL]],
        how="inner",
        on=Columns.TWO_LETTER_STATE_CODE,
    )
